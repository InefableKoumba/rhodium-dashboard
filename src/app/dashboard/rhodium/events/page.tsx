...
export default async function Page() {
  ...
  const { data: events } = (await response.json()) as { data: EventResponseInterface[] };
      ...
      <div className="p-8">
        <EventsTable events={
          events.map((event) => ({
            id: event.id,
            ...event.attributes,
            coverImage: event.attributes?.coverImage?.data ? process.env.NEXT_STORAGE_BUCKET_URL!.concat(
              event.attributes?.coverImage?.data?.attributes.url as string
            ) : undefined,
            images: event.attributes?.images?.data?.map(
              (image) => process.env.NEXT_STORAGE_BUCKET_URL!.concat(image?.attributes?.url as string)
            ),
            videos: event.attributes?.videos?.data?.map(
              (video) => process.env.NEXT_STORAGE_BUCKET_URL!.concat(video?.attributes?.url as string)
            ),
          }))
        } />
      </div>
